<template>
	<view class="diy_details diy_div_health_reporting">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','reporting_date')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>上报日期:</span>
						</view>
							<view class="diy_field diy_date">
							<text>
								{{ $toTime(obj["reporting_date"],"yyyy-MM-dd") }}
							</text>
						</view>
						</view>
					<view v-if="$check_field('get','temperature_today')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>今日体温:</span>
						</view>
							<view class="diy_field diy_text">
							<text>
								{{obj["temperature_today"]}}
							</text>
						</view>
						</view>
					<view v-if="$check_field('get','health_code')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>健康码:</span>
						</view>
							<view class="diy_field diy_img">
							<image style="width:100%;height:auto;" :src="$fullUrl(obj['health_code'])" mode="widthFix"></image>
						</view>
						</view>
					<view v-if="$check_field('get','is_it_uncomfortable')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>是否不适:</span>
						</view>
							<view class="diy_field diy_text">
							<text>
								{{obj["is_it_uncomfortable"]}}
							</text>
						</view>
						</view>
					<view v-if="$check_field('get','unwell_symptoms')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>不适症状:</span>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/health_reporting/get_obj", {
					health_reporting_id:this.query.health_reporting_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["health_reporting_id"] !==0) {
