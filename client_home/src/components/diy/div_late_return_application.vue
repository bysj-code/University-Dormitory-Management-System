<template>
	<view class="diy_details diy_div_late_return_application">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','late_return_time')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>晚归时间:</span>
						</view>
							<view class="diy_field diy_datetime">
							<text>
								{{ $toTime(obj["late_return_time"],"yyyy-MM-dd hh:mm:ss") }}
							</text>
						</view>
						</view>
					<view v-if="$check_field('get','reasons_for_late_return')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>晚归原因:</span>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/late_return_application/get_obj", {
					late_return_application_id:this.query.late_return_application_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["late_return_application_id"] !==0) {
...
<style scoped>
	.div_late_return_application_details {}
