<template>
	<view class="page_notice_announcement diy_list diy_list_page div_list_tml" id="notice_announcement_list">
		<!-- 筛选模块(开始) -->
		<view>
						...
						<view class="">
							<!-- 搜索栏 -->
								...
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
																			...
																			<uni-search-bar placeholder="搜索发布时间" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'release_time')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
															<!-- /搜索栏 -->
						</view>
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?notice_announcement_id=' + o['notice_announcement_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
	 ...
	 */
	get_praise(list){
		this.$get("~/api/praise/count_group?groupby=source_id&source_table=notice_announcement", {}, (res) => {
			if(res.result){
				res.result.map((o)=>{
					for(var i = 0;i < list.length; i++){
						var oj = list[i];
						if(oj["notice_announcement_id"] === o["source_id"]){
