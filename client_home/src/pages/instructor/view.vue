        ...
        <view>
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','trainee_no') || ($check_field('add','trainee_no') || $check_field('set','trainee_no'))" label="辅导员工号" name="trainee_no">
                            <uni-easyinput type="text" v-model="form['trainee_no']" v-if="user_group === '管理员' || (form['instructor_id'] && $check_field('set','trainee_no')) || (!form['instructor_id'] && $check_field('add','trainee_no'))" :disabled="disabledObj['trainee_no_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','trainee_no')">
                  {{ form['trainee_no'] }}
                </text>
                          </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','name_of_instructor') || ($check_field('add','name_of_instructor') || $check_field('set','name_of_instructor'))" label="辅导员姓名" name="name_of_instructor">
                            <uni-easyinput type="text" v-model="form['name_of_instructor']" v-if="user_group === '管理员' || (form['instructor_id'] && $check_field('set','name_of_instructor')) || (!form['instructor_id'] && $check_field('add','name_of_instructor'))" :disabled="disabledObj['name_of_instructor_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','name_of_instructor')">
                  {{ form['name_of_instructor'] }}
                </text>
                          </uni-forms-item>
...
export default {
  ...
  data() {
    return {
      ...
      is_password: true,
      field: "instructor_id",
      url_add: "~/api/instructor/add?",
      url_set: "~/api/instructor/set?",
      url_get_obj: "~/api/instructor/get_obj?",
      url_upload: "~/api/instructor/upload?",
      ...
      form: {
            "trainee_no":  '', // 辅导员工号
                    "name_of_instructor":  '', // 辅导员姓名
                                "user_id": 0,
                "instructor_id": 0, // ID
                
              },
          disabledObj:{
                        "trainee_no_isDisabled": false,
                                "name_of_instructor_isDisabled": false,
                                  },
                                                              }
  },
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/instructor/upload?'),
