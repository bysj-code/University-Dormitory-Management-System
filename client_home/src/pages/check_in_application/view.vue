          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','student_no') || ($check_field('add','student_no') || $check_field('set','student_no'))" label="学生学号" name="student_no">
                  ...
                  v-if="user_group === '管理员' || (form['check_in_application_id'] && $check_field('set','student_no')) || (!form['check_in_application_id'] && $check_field('add','student_no'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['student_no']"
                  :localdata="list_user_student_no"
              ...
              <uni-forms-item v-if="$check_field('get','student_name') || ($check_field('add','student_name') || $check_field('set','student_name'))" label="学生姓名" name="student_name">
                            <uni-easyinput type="text" v-model="form['student_name']" v-if="user_group === '管理员' || (form['check_in_application_id'] && $check_field('set','student_name')) || (!form['check_in_application_id'] && $check_field('add','student_name'))" :disabled="disabledObj['student_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','instructor') || ($check_field('add','instructor') || $check_field('set','instructor'))" label="辅导员" name="instructor">
                  ...
                  v-if="user_group === '管理员' || (form['check_in_application_id'] && $check_field('set','instructor')) || (!form['check_in_application_id'] && $check_field('add','instructor'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['instructor']"
                  :localdata="list_user_instructor"
              ...
              <uni-forms-item v-if="$check_field('get','building_name') || ($check_field('add','building_name') || $check_field('set','building_name'))" label="楼栋名称" name="building_name">
                            <uni-easyinput type="text" v-model="form['building_name']" v-if="user_group === '管理员' || (form['check_in_application_id'] && $check_field('set','building_name')) || (!form['check_in_application_id'] && $check_field('add','building_name'))" :disabled="disabledObj['building_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','dormitory_room_number') || ($check_field('add','dormitory_room_number') || $check_field('set','dormitory_room_number'))" label="宿舍房号" name="dormitory_room_number">
                            <uni-easyinput type="text" v-model="form['dormitory_room_number']" v-if="user_group === '管理员' || (form['check_in_application_id'] && $check_field('set','dormitory_room_number')) || (!form['check_in_application_id'] && $check_field('add','dormitory_room_number'))" :disabled="disabledObj['dormitory_room_number_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','dormitory_room_number')">
                  {{ form['dormitory_room_number'] }}
                </text>
                          </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','date_of_check_in') || ($check_field('add','date_of_check_in') || $check_field('set','date_of_check_in'))" label="入住日期" name="date_of_check_in">
                    <uni-datetime-picker v-if="user_group === '管理员' || (form['check_in_application_id'] && $check_field('set','date_of_check_in')) || (!form['check_in_application_id'] && $check_field('add','date_of_check_in'))" v-model="form['date_of_check_in']" type="date" :disabled="disabledObj['date_of_check_in_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','date_of_check_in')">
                  {{ form['date_of_check_in'] }}
                </text>
                  </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','check_in_instructions') || ($check_field('add','check_in_instructions') || $check_field('set','check_in_instructions'))" label="入住说明" name="check_in_instructions">
                    <uni-easyinput type="textarea" v-model="form['check_in_instructions']" v-if="user_group === '管理员' || (form['check_in_application_id'] && $check_field('set','check_in_instructions')) || (!form['check_in_application_id'] && $check_field('add','check_in_instructions'))" :disabled="disabledObj['check_in_instructions_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','check_in_instructions')">
    ...
    return {
      ...
      form: {
            "student_no": 0, // 学生学号
                    ...
                    "date_of_check_in": this.$toTime(new Date().getTime(), "yyyy-MM-dd"),
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/check_in_application/upload?'),
    ...
    get_obj_before(param){
      ...
      for (var i=0;i<arr.length;i++){
        this.disabledObj[arr[i] + '_isDisabled'] = true
                                                      ...
                                                      if (this.form["date_of_check_in"] && JSON.stringify(this.form["date_of_check_in"]).indexOf("-")===-1) {
