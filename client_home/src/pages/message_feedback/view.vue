          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','student_no') || ($check_field('add','student_no') || $check_field('set','student_no'))" label="学生学号" name="student_no">
                  ...
                  v-if="user_group === '管理员' || (form['message_feedback_id'] && $check_field('set','student_no')) || (!form['message_feedback_id'] && $check_field('add','student_no'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['student_no']"
                  :localdata="list_user_student_no"
              ...
              <uni-forms-item v-if="$check_field('get','student_name') || ($check_field('add','student_name') || $check_field('set','student_name'))" label="学生姓名" name="student_name">
                            <uni-easyinput type="text" v-model="form['student_name']" v-if="user_group === '管理员' || (form['message_feedback_id'] && $check_field('set','student_name')) || (!form['message_feedback_id'] && $check_field('add','student_name'))" :disabled="disabledObj['student_name_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','student_name')">
                  {{ form['student_name'] }}
                </text>
                          </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','student_phone') || ($check_field('add','student_phone') || $check_field('set','student_phone'))" label="学生电话" name="student_phone">
                            <uni-easyinput type="text" v-model="form['student_phone']" v-if="user_group === '管理员' || (form['message_feedback_id'] && $check_field('set','student_phone')) || (!form['message_feedback_id'] && $check_field('add','student_phone'))" :disabled="disabledObj['student_phone_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','student_phone')">
                  {{ form['student_phone'] }}
                </text>
                          </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','title') || ($check_field('add','title') || $check_field('set','title'))" label="标题" name="title">
                            <uni-easyinput type="text" v-model="form['title']" v-if="user_group === '管理员' || (form['message_feedback_id'] && $check_field('set','title')) || (!form['message_feedback_id'] && $check_field('add','title'))" :disabled="disabledObj['title_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','feedback') || ($check_field('add','feedback') || $check_field('set','feedback'))" label="反馈意见" name="feedback">
                    <uni-easyinput type="textarea" v-model="form['feedback']" v-if="user_group === '管理员' || (form['message_feedback_id'] && $check_field('set','feedback')) || (!form['message_feedback_id'] && $check_field('add','feedback'))" :disabled="disabledObj['feedback_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','feedback')">
                  {{ form['feedback'] }}
                </text>
                  </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','message_reply') || ($check_field('add','message_reply') || $check_field('set','message_reply'))" label="留言回复" name="message_reply">
                    <uni-easyinput type="textarea" v-model="form['message_reply']" v-if="user_group === '管理员' || (form['message_feedback_id'] && $check_field('set','message_reply')) || (!form['message_feedback_id'] && $check_field('add','message_reply'))" :disabled="disabledObj['message_reply_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','message_reply')">
                  {{ form['message_reply'] }}
                </text>
                  </uni-forms-item>
