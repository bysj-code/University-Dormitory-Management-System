          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','student_no') || ($check_field('add','student_no') || $check_field('set','student_no'))" label="学生学号" name="student_no">
                  ...
                  v-if="user_group === '管理员' || (form['repair_application_id'] && $check_field('set','student_no')) || (!form['repair_application_id'] && $check_field('add','student_no'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['student_no']"
                  :localdata="list_user_student_no"
              ...
              <uni-forms-item v-if="$check_field('get','student_name') || ($check_field('add','student_name') || $check_field('set','student_name'))" label="学生姓名" name="student_name">
                            <uni-easyinput type="text" v-model="form['student_name']" v-if="user_group === '管理员' || (form['repair_application_id'] && $check_field('set','student_name')) || (!form['repair_application_id'] && $check_field('add','student_name'))" :disabled="disabledObj['student_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','housemaster') || ($check_field('add','housemaster') || $check_field('set','housemaster'))" label="宿管" name="housemaster">
                  ...
                  v-if="user_group === '管理员' || (form['repair_application_id'] && $check_field('set','housemaster')) || (!form['repair_application_id'] && $check_field('add','housemaster'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['housemaster']"
                  :localdata="list_user_housemaster"
              ...
              <uni-forms-item v-if="$check_field('get','building_name') || ($check_field('add','building_name') || $check_field('set','building_name'))" label="楼栋名称" name="building_name">
                            <uni-easyinput type="text" v-model="form['building_name']" v-if="user_group === '管理员' || (form['repair_application_id'] && $check_field('set','building_name')) || (!form['repair_application_id'] && $check_field('add','building_name'))" :disabled="disabledObj['building_name_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','building_name')">
                  {{ form['building_name'] }}
                </text>
                          </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','dormitory_room_number') || ($check_field('add','dormitory_room_number') || $check_field('set','dormitory_room_number'))" label="宿舍房号" name="dormitory_room_number">
                            <uni-easyinput type="text" v-model="form['dormitory_room_number']" v-if="user_group === '管理员' || (form['repair_application_id'] && $check_field('set','dormitory_room_number')) || (!form['repair_application_id'] && $check_field('add','dormitory_room_number'))" :disabled="disabledObj['dormitory_room_number_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','dormitory_room_number')">
                  {{ form['dormitory_room_number'] }}
                </text>
                          </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','items_for_repair') || ($check_field('add','items_for_repair') || $check_field('set','items_for_repair'))" label="报修物品" name="items_for_repair">
                            <uni-easyinput type="text" v-model="form['items_for_repair']" v-if="user_group === '管理员' || (form['repair_application_id'] && $check_field('set','items_for_repair')) || (!form['repair_application_id'] && $check_field('add','items_for_repair'))" :disabled="disabledObj['items_for_repair_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','items_for_repair')">
                  {{ form['items_for_repair'] }}
                </text>
                          </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','repair_reason') || ($check_field('add','repair_reason') || $check_field('set','repair_reason'))" label="报修原因" name="repair_reason">
                    <uni-easyinput type="textarea" v-model="form['repair_reason']" v-if="user_group === '管理员' || (form['repair_application_id'] && $check_field('set','repair_reason')) || (!form['repair_application_id'] && $check_field('add','repair_reason'))" :disabled="disabledObj['repair_reason_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','repair_reason')">
                  {{ form['repair_reason'] }}
                </text>
                  </uni-forms-item>
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/repair_application/upload?'),
