        ...
        <view>
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','hanguan_job_number') || ($check_field('add','hanguan_job_number') || $check_field('set','hanguan_job_number'))" label="宿管工号" name="hanguan_job_number">
                            <uni-easyinput type="text" v-model="form['hanguan_job_number']" v-if="user_group === '管理员' || (form['housing_management_users_id'] && $check_field('set','hanguan_job_number')) || (!form['housing_management_users_id'] && $check_field('add','hanguan_job_number'))" :disabled="disabledObj['hanguan_job_number_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','hanguan_job_number')">
                  {{ form['hanguan_job_number'] }}
                </text>
                          </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','name_of_the_housekeeper') || ($check_field('add','name_of_the_housekeeper') || $check_field('set','name_of_the_housekeeper'))" label="宿管姓名" name="name_of_the_housekeeper">
                            <uni-easyinput type="text" v-model="form['name_of_the_housekeeper']" v-if="user_group === '管理员' || (form['housing_management_users_id'] && $check_field('set','name_of_the_housekeeper')) || (!form['housing_management_users_id'] && $check_field('add','name_of_the_housekeeper'))" :disabled="disabledObj['name_of_the_housekeeper_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','name_of_the_housekeeper')">
                  {{ form['name_of_the_housekeeper'] }}
                </text>
                          </uni-forms-item>
...
export default {
  ...
  data() {
    return {
      ...
      group_table: "housing_management_users",
      is_password: true,
      field: "housing_management_users_id",
      url_add: "~/api/housing_management_users/add?",
      url_set: "~/api/housing_management_users/set?",
      url_get_obj: "~/api/housing_management_users/get_obj?",
      url_upload: "~/api/housing_management_users/upload?",
      ...
      form: {
            "hanguan_job_number":  '', // 宿管工号
                    "name_of_the_housekeeper":  '', // 宿管姓名
                                "user_id": 0,
                "housing_management_users_id": 0, // ID
                
              },
          disabledObj:{
                        "hanguan_job_number_isDisabled": false,
                                "name_of_the_housekeeper_isDisabled": false,
                                  },
                                                              }
  },
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/housing_management_users/upload?'),
    ...
    submit_sub(form_sub){
      if(form_sub["housing_management_users_id"]){
        // 提交事件
        this.$post("~/api/" + this.group_table + "/set?housing_management_users_id=" + form_sub["housing_management_users_id"],form_sub,(res)=>{
    ...
    is_view(){
      ...
      if(!bl){
        bl = this.$check_action('/housing_management_users/table','add');
        console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
      }
      if(!bl){
        bl = this.$check_action('/housing_management_users/table','set');
        console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
      }
      if(!bl){
        bl = this.$check_action('/housing_management_users/view','add');
        console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
      }
      if(!bl){
        bl = this.$check_action('/housing_management_users/view','set');
        console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
      }
      if(!bl){
        bl = this.$check_action('/housing_management_users/view','get');
