<template>
  <view class="page_health_reporting diy_table diy_table_tml" id="health_reporting_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                        <uni-forms-item label="上报日期" name="reporting_date">
                      <uni-easyinput type="text" v-model="query.reporting_date" placeholder="上报日期" />
                    </uni-forms-item>
                        <uni-forms-item label="是否不适" name="is_it_uncomfortable">
                 <uni-data-select placeholder="请选择"
                        v-model="query['is_it_uncomfortable']"
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/health_reporting/view?" v-show="user_group == '管理员' || $check_action('/health_reporting/table','add') || $check_action('/health_reporting/view','add')">添加</navigator>
						...
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','instructor')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>辅导员</span>
							</view>
															<view class="diy_field diy_uid text">
								<span>
									{{ get_user_instructor(o['instructor']) }}
								</span>
								</view>
													</view>
					</view>
									<view class="col" v-if="1 && $check_field('get','reporting_date')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>上报日期</span>
							</view>
								...
								<span>
									{{ $toTime(o["reporting_date"] ,"yyyy-MM-dd") }}
								</span>
								</view>
													</view>
					</view>
									<view class="col" v-if="1 && $check_field('get','temperature_today')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>今日体温</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','health_code')">
						<view class="view">
							<view class="diy_title" v-if=" false">
								<span>健康码</span>
							</view>
															<view class="diy_field diy_img">
									<img :src="$fullUrl(o['health_code'])" width="100%" height="100" />
								</view>
													</view>
					</view>
									<view class="col" v-if="1 && $check_field('get','is_it_uncomfortable')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>是否不适</span>
							</view>
															<view class="diy_field diy_text text">
								<span>
									{{ o["is_it_uncomfortable"] }}
								</span>
								</view>
													</view>
					</view>
									<view class="col" v-if="1 && $check_field('get','unwell_symptoms')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>不适症状</span>
							</view>
				...
				<view class="col">
					<view class="view">
						<view class="create_time_block diy_filed diy_time">
                          ...
                          <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/health_reporting/table','del') || $check_action('/health_reporting/view','del')">删除</button>
                        </view>
					</view>
				</view>
				<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/health_reporting/view?' + field + '=' + o[field]"
						v-show="user_group == '管理员' || $check_action('/health_reporting/table','set') || $check_action('/health_reporting/view','set') || $check_action('/health_reporting/view','get')" class="btn btn-primary">详情</navigator>
					                    									</view>
			</view>
		</view>
	</view>
		...
		data() {
			return {
                ...
                list_user_instructor: [],
                                                // 是否不适列表
                list_is_it_uncomfortable: [{value:"",text:"全部"}],
                			}
		},
		methods: {
          search() {
