<template>
	<view class="page_health_reporting diy_list diy_list_page div_list_tml" id="health_reporting_list">
		<!-- 筛选模块(开始) -->
		<view>
			<view class="container">
				<view>
					<view>
						<view class="">
							<!-- 搜索栏 -->
																<uni-search-bar placeholder="搜索上报日期" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'reporting_date')">
				...
				<view>
					<view>
						<view class="tab_view">
							<!-- 筛选栏 -->
																						<uni-section title="是否不适" type="line">
							<uni-data-select
									@change="change_is_it_uncomfortable"
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?health_reporting_id=' + o['health_reporting_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
			...
			return {
				url_get_list: "~/api/health_reporting/get_list?like=0",
					...
					list_user_instructor: [],
														// 是否不适选项列表
				list_is_it_uncomfortable: [{value:"全部",text:"全部"}],
	...
	get_user_instructor(id){
		...
		return ret;
						...
						// 改变分类标签
	change_is_it_uncomfortable(val) {
		if (val === "全部" || val === "") {
			this.query.is_it_uncomfortable = "";
		}else{
			this.query.is_it_uncomfortable = val
		}
		this.search();
	},
